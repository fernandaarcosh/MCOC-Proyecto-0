...
#Calculo de errores entre el valor real y exacto, y el valor entregado por el programa tanto en 32 y 64 bits
error_32_r = []
error_64_r = []
...
i = 0
while i < N:
    error_32_r.append((abs(valores_32[i] - valor_real[i]))/valor_real[i])
    error_64_r.append((abs(valores_64[i] - valor_real[i]))/valor_real[i])
    i+=1

j = 0
while j < N:
    error_32_e.append((abs(valores_32[j] - valor_exacto[j]))/valor_exacto[j])
    error_64_e.append((abs(valores_64[j] - valor_exacto[j]))/valor_exacto[j])#como es de esperar este error debiese ser 0
    j+=1
...
plt.xlabel("a")
plt.ylabel("Error relativo f(x) = a**3")
plt.figure(1)
